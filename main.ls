...
get-file = (files, year, map, count, idx) ->
  ...
  request url .pipe fs.createWriteStream name .on \finish, ->
    ...
    for line in lines
      ...
      if line.0.index-of(\")==0 => total.{}[year][town] = parseInt line.1
...
download = (idx) ->
  if datasrc.length <= idx => 
    ...
    fs.write-file-sync \total.json, JSON.stringify(total)
    return
  console.log "retrieve year: #{datasrc[idx]0}"
  (e,r,b) <- request datasrc[idx]1
  data = b.split \\n
  #year = ([findyear.exec(line) for line in data]filter (->it) .map -> it.1)0
  year = datasrc[idx]0
